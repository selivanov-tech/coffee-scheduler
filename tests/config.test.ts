...
describe("parseConfig", () => {
  it("accepts a complete environment and applies defaults", () => {
    ...
    expect(cfg.caps.webhookTimeoutMs).toBeGreaterThan(
      cfg.caps.agentTurnDeadlineMs,
    );
  });
  ...
  it("rejects an invalid service-account JSON string", () => {
    expect(() =>
      parseConfig({ ...fullEnv, GOOGLE_SERVICE_ACCOUNT_JSON: "not-json" }),
    ).toThrow();
  });
